...
And the term $\means{\mathbf{y}}$ is zero for the REV used in ordered porous media for symmetric reasons and the second term can be shown either to be small with the argument that is order $\epsilon^2$ or the filter function can be chosen in order that $m*\mathbf{y}\mathbf{y} =0$ strictly.

As we recall from section ??? the triagòe shaped filter satisfy this hypotesis.