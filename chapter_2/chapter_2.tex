...
\Red{We }have already introduced, in the previous chapter, that the volume averaging method can be used to derive a macroscopic description of the fluid dynamic.
The final version of the equations is valid everywhere in the porous medium domain, and not only in the fluid phase.
Theoretical aspect of the volume averaging method applied to fluids flow in porous media can be found in \citet{whitaker2013method}, \citet{whitaker1986flow}, \citet{whitaker1996forchheimer}, \citet{quintard1994transport1}, \citet{quintard1994transport2}, \citet{quintard1994transport3}, \citet{quintard1994transport4}, \citet{quintard1994transport5} and many others contributions that are introduced in the next chapter.
The various stages necessary to derive the local average version of the fluid dynamic equations are listed in the following text.
...
\item Decomposition of fields in an "average plus perturbation" manner
...
The shape of the volume used in the averaging operations are enclosed in continuous lines; $V|_{\mathbf{x}}$ indicate the volume with centroid $\mathbf{x}$ and $V_{\beta}|_{\mathbf{x}}$ indicate the fluid volume fraction inside the latter.
...
Let $\psi_{\beta}$ be a an arbitrary order tensors (scalar, vector or second order tensor) defined in the fluid phase of the volume $V$ with $x$ as centroid.
...
\Red{We }define two different volume averaging operators; the \textit{intrinsic average} indicated as $\meani{.}$ reads:
...
The difference between the two formulation is that the former takes into account the actual fluid fraction in averaging the filed instead of the pure size of the total volume.

In order to use a less heavy notation, the subscript $|_{\mathbf{x}}$ is dropped in the following procedure, but \Red{we }keep in mind that the volume averaged quantities are explicitly dependent on the center position of the volume as both averaging operators are defined as a volume integral.
...
The notation is further simplified if a constant filter is considered ($m=1/V$), in such case it is possible to drop it from the the average operators.
...
\begin{equation}
	...
	\label{eq:porosity}
\end{equation}
which represent how much fluid is actually present inside the averaging volume, in other terms is an indication of how packed are the fibers of our porous media.
...
Sometimes in the following text \Red{we }refer to the volume in which the average procedure is applied as REV, \textit{reference elementary volume}.
For disordered porous media a spherical volume is the most appropriate, and the REV size ($\ell$) should satisfy the length scale constraint:
$$
...
$$
that can be reinterpreted as the separation of scale parameter in multiple scale analysis $\epsilon = \ell/L \ll 1$.
...
The size of it ($\ell$) should be chosen with the above specifications, these length scales constraints assure that the volume is large enough that periodic boundary condition can be applied in the exterior of the volume but still be able to capture all the phenomena that take place at the micro-scale (scale $\ell_{\beta}$).
...
\Red{We }have already said that the function can help to attenuate variation of the averaged fields due to geometrical fluctuations of the porous medium.
It has in fact the property of a low-pass filter for the perturbations fields.

The filter function can plays an important role also in the interpretation of the averaged equation in fact (as showed later) in order to retrieve a local form of the VANS equations \Red{we }have to be able to make the statements:
\begin{equation}
...
In appendix \ref{ch:appendix_a} at the end of this chapter \Red{we }further explain the details of the approximation needed to derive equations \ref{eq:well_behaved}.
...
\begin{cases}
(\ell/2 - |\mathbf{y}|) \qquad |\mathbf{y}| \leqslant r_0\\
0 \qquad |\mathbf{y}|> r_0
\end{cases}
\end{eqnarray}

Whichever is the choice on the $m$ shape, \citet{davit2017technical} has re-formulated recently the required hypothesis that a $m$ function should satisfy; without entry in the details (that is beyond the aim of this paragraph) \Red{we }present some of the most important generalizations from the above works that should apply to any choice of the function $m$:
\begin{itemize}
	...
	\item $(m \, \mathcal{P}^j(y))*\psi_{\beta}
	\begin{cases}
...
Where $\mathcal{P}^j(y)$ is a polynomial of order $j$ in the variable $y$.
In the last requirement \Red{we }have used the fact that the average operation can also be defined as a convolution product between the filter function and the flow field quantities, \citet{marle1982macroscopic}:
$$
\means{\psi_{\beta}}|_{\mathbf{x}} = \dfrac{1}{V} \int_{\volb(\mathbf{x})} m(\mathbf{y}) \psi_{\beta}(\mathbf{x}+\mathbf{y}, t) d \, \volb = m*\psi_{\beta}
$$

Even though \Red{we }have shown the importance of the filter function shape, it should be clear that older literature works that had implicitly used the $m^{\sqcap}$ are not wrong, even in case of periodic ordered porous media.
In fact if \Red{we }assume that the field are well behaved (even without checking if the filter or the REV size are the good one) \Red{we }assume that the equation \eqref{eq:well_behaved} olds and that some simplifications of the averaging operators are permitted.
...
In the following derivation of the equation \Red{we }do not formally include any filter function inside the averaged operators in order to not make the notation heavier.
However \Red{we }indicate in the following text where this hypothesis on the filter is formally needed to derive the equation.

\subsection{Theorems involving derivatives of spatial averaged quantities}

In this paragraph \Red{we }report some the theorems related to the volume averaging technique. The purpose to these theorems is to invert the sequence of the  derivative and volume average operation.
...
\begin{corollary}
	Applying the theorem \eqref{th:spat_avg} to a constant field $\psi_{\beta} = 1$ \Red{we }obtain:
...
\Red{We }start by finding the averaged version of the continuity equation in \eqref{eq:NS}:
...
Applying theorem \eqref{th:spat_avg} to the previous equation \Red{we }get:
$$
...
Now \Red{we }rewrite the last term as if it was a result of the Reynolds transport theorem applied to a constant unitary scalar field:
$$=\nabla \cdot \means{\vb} + \derp{}{t} \dfrac{1}{V} \int_{\volb} \;dV  - \dfrac{1}{V} \int_{\volb} \derp{1}{t} \;dV $$
This results can be simplified; in fact the last integral is zero due to the time derivation of a constant field. The first term can be further developed, obtaining finally the averaged continuity equation:
...
In this paragraph \Red{we }compute the average version of the momentum equation in \eqref{eq:NS}:
...
In order to keep the procedure readable \Red{we }show the development of each term separately, in the same order as they appear in equation \eqref{eq:mom_1}.

\subsubsection{Temporal derivative term}
Using theorem \eqref{th:transport} \Red{we }can rewrite the first term of the equation as:
...
\begin{equation}
\means{-\dfrac{1}{\rho_{\beta}} \nabla \pb } = -\dfrac{1}{\rho_{\beta}} \nabla \means{\pb} -\dfrac{1}{V} \int_{A_{\beta \sigma}} \left( \dfrac{\pb}{\rho_{\beta}} \mathbf{I} \right) \cdot \nbs \;dA,
\end{equation}

\subsubsection{Diffusion term}
Here \Red{we }fist use the identity $\nabla^2 = \nabla \cdot (\nabla)$ (laplacian = div(grad)), then \Red{we }apply the theorem \ref{th:vec_spat_avg} directly to this expansion to get:
...
Before continue the development \Red{we }group all the various terms together:
...
In the next section these \Red{we }show how to thereat these two terms in order make them function of the only averaged quantities.

\subsection{Length scale decomposition}
In order to finally get the average version of the problem \eqref{eq:NS} \Red{we }make use of the decomposition proposed by \citet{gray1975derivation}:
...
Using the above results \Red{we }can modify the non-homogeneous term in equation \eqref{eq:mom_2}:
...
For each integral term of \ref{eq:mom_2} \Red{we }also apply the same field decomposition:

\begin{eqnarray}
\dfrac{1}{V} \int_{A_{\beta \sigma}}  - \left( \dfrac{\pb}{\rho_{\beta}} \mathbf{I} \right) \cdot \nbs \;dA &=& \dfrac{1}{V} \int_{A_{\beta \sigma}}  -\dfrac{1}{\rho_{\beta}} \left(\meani{\pb} \mathbf{I}  + \pbt \mathbf{I}\right) \cdot \nbs \;dA = \nonumber \\
&=& +\dfrac{1}{\rho_{\beta}}\nabla\varepsilon \meani{\pb} - \dfrac{1}{V} \int_{A_{\beta \sigma}} \left( \dfrac{\pbt}{\rho_{\beta}} \mathbf{I} \right) \cdot \nbs \;dA
\end{eqnarray}
...
In order to get the intrinsic average formulation \Red{we }use the relation \eqref{eq:means_vs_meani} to express surface averaged quantities in terms of intrinsic ones.

\Red{We }first start withe the continuity equation that became:
$$
\nabla \cdot (\varepsilon\meani{\vb} ) + \derp{\varepsilon}{t}= 0
$$

For the momentum equation \Red{we }start from the temporal derivative term that became:
$$
\derp{\means{\vb}}{t} = \derp{(\varepsilon\meani{\vb})}{t} = \derp{\varepsilon}{t}\meani{\vb} + \varepsilon \derp{\meani{\vb}}{t}
$$

Applying the same relation to the viscous term \Red{we }get:
...
After the proper simplification \Red{we }have the final versions of the Navier-Stokes system of equations \eqref{eq:NS} using intrinsic quantities:
...
In the next paragraphs \Red{we }develop closure formulations of these terms. In order to do be clear \Red{we }name these terms as \textit{sub-filter stresses} $\boldsymbol{\zeta}$ and \textit{microscopic force} at the fluid-solid interface $\mathbf{F}^m$:
...
There is no simple representation for $\mathbf{F}^m$ if \Red{we }include the terms that presents gradients of the porosity ($\nabla \varepsilon$).
Although since \Red{we }are interested in developing a \textbf{local} closure problem, that will depend on the geometry of each REV, is possible to simplify these terms.
This means that the closure problems are not correct at the interface between a porous medium and a free fluid; but in the last chapter \Red{we }show that even though \Red{we }are indeed committing an error, \Red{we }can still use the same closure problems and, still obtain good results.

In order to develop a closure problem \Red{we }recall the decomposition by \citet{gray1975derivation}:
\begin{equation}
\psi_{\beta} = \meani{\psi_{\beta}} + \tilde{\psi}_{\beta}
\end{equation}

The continuity equation valid at the microscopic scale became $ \nabla \cdot  \meani{\vb}=0$ after in case of constant porosity.
\Red{We }subtract this last equation from the continuity equation valid for the point velocity:
...
expanding the first velocity with the decomposition \eqref{eq:gray}, and making the proper simplification, \Red{we }obtain the continuity equation for the perturbations:
...
To continue the development \Red{we }first divide the momentum equation present in system \ref{eq:vans_mom_1} by the permeability $\varepsilon$, and \Red{we }apply also the hypothesis of constant porosity:
\begin{eqnarray}
...
Using the same procedure as above, \Red{we }subtract the above momentum equation from the point field one obtaining:
\begin{eqnarray}
...
Now in order to simplify the above equations \Red{we }introduce these length-scale estimates:
...
\citet{whitaker1996forchheimer} state clearly that there is no easy way to define a \textit{local} closure problem when the relation $\ell \ll \delta$ does not holds; in any case here \Red{we }suppose it correct to fulfill the derivation of the local closure problem although \Red{we }keep in mind this problem holds only far from region where the porosity field change (even if in the last chapter \Red{we }show that relaxing this hypothesis still produce good results).

Continuing with the order estimates \Red{we }can neglect some of the terms based on the estimation of different order of magnitude:
\begin{eqnarray}
...
In the last estimation \Red{we }have assumed that the time scale associated respectively with the micro and macro-scale are $t = \ell/\meani{\vb}$ and $T =L/\meani{\vb}$.
...
The boundary condition at the interface is valid only for rigid porous media; in this section \Red{we }derive the closure problem considering the solid phase as rigid althoug in section \ref{ph:moving} \Red{we }show how to extend the model in case of moving porous media.
The above system is still defined on all the porous domain and so \Red{we }would like to find a way to reduce its size and still obtain the same results.
...
Although is possible to use Green function to solve the problem in this form \citet{wood2013volume}, \Red{we }are interested in a \textit{local} closure problem.
...
Such hypothesis is consistent with periodic order porous media because \Red{we }have already stated that the variations connected to mean averaged quantities can be neglected inside the REV (see paragraph \ref{ch:appendix_a}).
...
Now \Red{we }introduce the closure variables $\mathbf{R}$, $\mathbf{r}$, for which \Red{we }made the hypothesis that:
\begin{eqnarray}
...
Is very important to pinpoint that the last assumption is crucial since \Red{we }imply a linear correlation between the micro and macro-scale fields.
...
Since \Red{we }are free to define the tensor $\mathbf{R}$  and the vector $\mathbf{r}$ as \Red{we }wish, \Red{we }chose to define it by means of the following problem:
...
With  $\mathbf{R}$ and $\mathbf{r}$ specified as above \Red{we }can prove that $\boldsymbol{\xi}$ is zero and $\gamma$ a constant \cite{whitaker1996forchheimer}.
...
In order to simplify it \Red{we }decompose the problem in two parts, the first one gives us the \textit{permeability tensor} and the second one the \textit{Forchheimer tensor}.

\Red{We }decompose $\mathbf{R}$ and $\mathbf{r}$ as:
...
\Red{We }are free to define $\mathbf{B}$ by means of the following system:
...
\Red{We }want to highlight that the problems for $R$, $B$ and $C$ are a free definition and they not arise from the direct substitution of \ref{eq:closure_viab_divison} and 	\ref{eq:closure_viab_hp} into the system \ref{eq:vans_mom_2}.
Injecting the decomposition \ref{eq:closure_viab_hp} inside the surface filter $\mathbf{F}^m$ \Red{we }get:
...
\end{equation}
in which is clear the equivalence between the description by means of the perturbation fields and the one that uses only the macroscopic  fields, \Red{we }refer to this equivalence as closure formulation.


\subsection{Sub-filter stresses $\boldsymbol{\zeta}$}

\Red{We }have already seen that yet another term need to be closed.
...
$$\boldsymbol{\zeta} = \nabla \cdot \left(\means{\vbt \vbt}\right)$$
Although \Red{we }have already seen in the previous paragraph that this term can be neglected, based on some length-scale argument, here \Red{we }want to explain briefly what this term represent and possibly when it can be important.
...
Although, \Red{we }speculate that this term can become important in situations involving elastic porous media such as canopy flow where when the interface experience sweep and injection of fluid at the interface.
In fact \citet{finnigan2000turbulence} and \citet{de2008effects} have shown that the turbulence spectrum is modified in the above cases and \Red{we }argue that the sub-filter stresses term can, probably, model this high frequency vorticies.

In order to better study this term \Red{we }need reliable full DNS inside the porous media at high pore Reynolds number, but such simulations are very expensive and almost un-existent in literature.
...
The problem is very complex and our work is not focused on the development of a new condition; besides, in this paragraph, \Red{we }want to explain our choice for the interface treatment over the many possible ones.
...
\Red{We }think that using a boundary condition at the interface is not a superior approach nor physically neither mathematically.
...
\citet{whitaker1986deformable} has also approached the problem using the volume average method tho homogenize either the solid and the fluid part.
Furthermore the closure problems that came out from the averaging procedure of the solid equation are very difficult to solve and; there are no following studies that neither clarify the problem nor confirm the formulation.

Following works by \citet{hussong2011continuum} and \citet{wang2015volume} showed how to extend the rigid case formulation of the VANS in case of elastic medium.
The only term that has to be changed is the closure for the microscopic force \eqref{eq:closure_final}, as a matter of fact the hypothesis of rigid porous media has been used in this only term.
They proposed a modification based on the physical interpretation of this term. In reality the force exerted by the fluid on the solid part has to take into account the solid velocity:
$$
\mathbf{F}^M = - \nub \varepsilon \mathbf{H}^{-1} \cdot \left[ \meani{\vb} - \meani{\ws} \right]
$$
but the physics behind the force generation is the same and so it should be the formulation of the closure.

This simple modification can be included in an \textit{hybrid homogenization approach}. As presented in paragraph \ref{ch:model_porous} to describe the dynamic of a poroelastic layer \Red{we }need a model for the fluid part, and one for the solid plus an interface condition; but since the dynamic of the solid is usually computationally cheap there is no need to use an homogenized model for this part.
Here \Red{we }propose an approach that is transparent to whatever solid model one want to use.
The pseudo-code fo the macroscopic algorithm is the one in \ref{algo:vans_elastic}.
\vspace{0.5cm}
...
\begin{algorithm}[H]
	...
	$n \longrightarrow 0$ \;
		...
		${\boldsymbol{\meani{\psi_{\beta}}}}_{n+1} \longrightarrow {\boldsymbol{\meani{\psi_{\beta}}}}_{t+ \Delta t}$\;
	}
...
\vspace{0.5cm}
Any solid model can be used in the model (for example Bernulli beam) and when solved a new porosity field and homogenized solid field can be computed in this manner.
...
The idea that \Red{we }want to express is that with just some slight modification in the fluid equations is possible to easily develop a macroscopic algorithm that can take into account moving fibrous media.
In the above \Red{we }also introduce (in line 7) the idea of a metamodel for the apparent permeability, this is needed because in chapter \ref{ch:4} \Red{we }will show how $\mathbf{H}$ is affected by the direction and intensity of the mean velocity field.
...
In the above sections \Red{we }have briefly talked about the results in equation \ref{eq:well_behaved} that \Red{we }recall here:
...
In order to show how the above expression can be simplified \Red{we }expand the averaged quantity $\means{\psi_{\beta}}|_{\mathbf{r}}$ over the centroid $\mathbf{x}$ using Taylor's polynomial:
$$
...
Now if \Red{we }put this expansion inside the averaging operator \Red{we }get:
$$
