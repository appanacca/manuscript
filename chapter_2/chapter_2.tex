...
Here we propose an approach that is transparent to whatever solid model one want to use.
...
\begin{algorithm}[H]
	...
	$n \longrightarrow 0$\;
		\While{$n < n_{max}$}{
			...
			Effective permeability metamodel:
			Relaxation ??
...
The idea that we want to express is that with just some slight modification in the fluid equations is possible to easily develop a macroscopic algorithm that can take into account moving fibrous media.
In the above we also introduce (in line 7??) the idea of a metamodel for the apparent permeability, this is needed because in chapter \ref{ch:4} we will show how $\mathbf{H}$ is affected by the direction and intensity of the mean velocity field.
