...
The weight function can play an important role also in the interpretation of the averaged equation. As showed later on, in order to retrieve a local form of the VANS equations, the following statement should in principle has to be true:
\begin{equation}
...
The importance of the weight function shape is clear, however older works in which the authors had implicitly used $m^{\sqcap}$ are not wrong.
As a matter of fact, if the assumption of well behaved fields hold \footnote{it means that the equation \eqref{eq:well_behaved} can be verified a posteriori} than the homogenized equations are the correct one.
...
The purpose of these theorems is to be able to invert the derivative and the volume average operation.
...
where $\tilde{\psi}_{\beta}$ is the microscopic scale contribution to add at the intrinsic volume average quantity $ \meani{\psi_{\beta}}$ to obtain the point value of the considered quantity $\psi_{\beta}$.
...
In order to overcome this problem in the next section the intrinsic version of these equations is finally computed. 
...
We first start with the continuity equation that became:
$$
\nabla \cdot (\varepsilon\meani{\vb} ) + \derp{\varepsilon}{t}= 0
$$

For the momentum equation we start from the temporal derivative term that becomes:
$$
...
First of all is important to pinpoint that the intrinsic momentum equation represents the force per unit volume. It means that it explicitly depends on the porosity of the medium. This is why the equation has terms involving gradients of the porosity field.
In application where porosity can vary spatially, like the interface of a porous medium, this formulation has the advantage to treat explicitly the interface non-homogeneities \footnote{further discussion of the interface treatment is presented in paragraph \ref{ch:interface}}.
...
In the next paragraphs a closure formulation of these terms is presented. In order to do be clear these two terms are named as \textit{sub-filter stresses} $\boldsymbol{\zeta}$ and \textit{microscopic force} $\mathbf{F}^m$:
...
The term $\mathbf{F}^m$ is referred as \textit{surface filter} since it is defined as an integral over the fluid-solid interface for the micro-scale fields, in this manner the perturbation fields are filtered out. It physically represent the force per unit mass that the fluid exerts on the solid porous media.
...
The continuity equation valid at the microscopic scale becomes $ \nabla \cdot  \meani{\vb}=0$ after in case of constant porosity.
...
The last relation is based on the fact that the porosity varies on scale $\delta$ that is much greater than $\ell$, this means $\ell \ll \delta$. \citet{valdes2013velocity} and \citet{ochoa1995momentum} make this argument considering that $\delta$ has the size of the zone in which the porosity varies, in case of an interface between a porous medium and a free fluid.
However, is important to state that this assumption does not holds at the interface of all the porous media geometry. As a matter of fact for ordered porous media $\varepsilon = O(\ell)$.
\citet{whitaker1996forchheimer} state clearly that there is no easy way to define a \textit{local} closure problem when the relation $\ell \ll \delta$ does not holds. Here we suppose that the relation holds, although we keep in mind that the problem is formally correct only far from regions where the porosity can change.
...
I can also be notice that in the above simplifications we have neglected terms that contains the small parameter $\epsilon$, or powers of this term. This last results is coherent with the multiple scale theory in which only zero order terms are used in the local closure problem formulation.

The order of magnitude estimation leaves use with:
\begin{eqnarray}
	\begin{cases}
		\vb \cdot \nabla \vbt = -\nabla \left(\dfrac{\pbt}{\rho_{\beta}}\right) + \nub \nabla^2 \vbt - \dfrac{1}{\volb} \displaystyle \int_{A_{\beta \sigma}} \left(-\dfrac{\pbt}{\rho_{\beta}} \mathbf{I}  + \nub \nabla \vbt \right) \cdot \nbs \;dA  \\
		\nabla \cdot \vbt = 0  \\
...
Although it is possible to use Green function to solve the problem in this form \citet{wood2013volume}, we are interested in a \textit{local} closure problem.
...
The problem stated in this manner becomes:
...
\end{eqnarray}
the above choice is just for convenience.
...
However this relation can be function of the space coordinate $x$ as explored later in chapter \ref{ch:4}.
...
It is possible to prove that with the above problem definition $\boldsymbol{\xi}$ is zero and $\gamma$ a constant (\citet{whitaker1996forchheimer}).
...
We want to highlight that the problems for $\mathbf{R}$, $\mathbf{B}$ and $\mathbf{C}$ are a free definition and they not arise from the direct substitution of \ref{eq:closure_viab_divison} and 	\ref{eq:closure_viab_hp} into the system \ref{eq:vans_mom_2}. Injecting the decomposition \ref{eq:closure_viab_hp} inside the surface filter $\mathbf{F}^m$ we get:
...
making the problem \eqref{eq:B_problem} becomes:
\begin{eqnarray}
	\begin{cases}
...
One of the most famous it is the Kozeny-Carman relation \citet{kozeny} and the modified Ergun equation \citet{transport2002bird}.
Extended version of this empirical formulation can be found in \citet{zampogna2016fluid} and \citet{yazdchi2012towards}.
However, the above relationships are always based on some regression from experiments or computation, and they are parameterized usually with the porosity and some geometrical characteristic of the medium.
The downsizes in using these simplified formulas is that their range of application can be restricted to very small Reynolds number and that the geometries used are most of the time very simple such as spheres, or 2D regular arranged cylinders. Such restrictions makes the local closure problem the main reliable source to compute the two tensors.
...
We have already seen that yet another term needs to be closed.
...
$$\boldsymbol{\zeta} = \nabla \cdot \left(\means{\vbt \vbt}\right)$$
Although we have already seen in the previous paragraph that this term can be neglected, based on some length-scale argument, here we want to explain briefly what this term represent and possibly when it can becomes important.
...
However, we speculate that this term can becomes important in situations involving elastic porous where sweep and injection of fluid can be observed at the interface.
...
\citet{hussong2011continuum} made a direct comparison, with a DNS simulation, for the porosity treatment concluding that the variation of the permeability is very important to have a good comparison with high fidelity computation.
...
$$
but the physic behind the force generation is the same and so it should be the formulation of the closure.
...
And introducing the decomposition \ref{eq:gray} the above results can be used to state that the perturbation fields have null averaged:
