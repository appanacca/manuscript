...
In the previous chapter has already been introduced how the volume averaging method, can be used to derive a macroscopic description of the fluid dynamic system of equations.
...
In order to use a less heavy notation, the subscript $|_{\mathbf{x}}$ is dropped in the following procedure, but should be keep in mind that the volume averaged quantities are explicitly dependent on the center position of the volume as both averaging operators are defined as a volume integral.
...
The authors above differentiate their results for ordered and disordered porous medium. They show that in each case a specific size and shape of the filter (and the volume) is needed.
Is common to refer to the volume in which the average procedure is applied as REV, meaning \textit{reference elementary volume}.
...
The size of the REV ($\ell$) should be chosen with the above specifications. These length scales constraints assure that the volume is large enough that periodic boundary condition can be applied in the exterior of the volume. Anyhow the REV size should also capture all the phenomena that take place at the micro-scale ($\ell_{\beta}$).
If the REV size is the correct one, increasing or decreasing its size, of a certain amount, should not change the averages quantities. The weight function can also help to attenuate variation of the averaged fields due to geometrical fluctuations of the porous medium; as a matter of fact it acts as a low-pass filter for the perturbations fields.

The filter function can plays an important role also in the interpretation of the averaged equation. As showed later on, in order to retrieve a local form of the VANS equations, the following statement should in principle be true:
\begin{equation}
\Big\langle \means{\psi_{\beta}}|_{\mathbf{r}}  \Big\rangle  |_{\mathbf{x}} = \means{\psi_{\beta}}|_{\mathbf{x}}
\label{eq:well_behaved}
\end{equation}
It means that the averaged fields contains small variations at the micro-scale (inside the averaging volume $V$).
...
\begin{itemize}
	...
	\item satisfy: $(m \, \mathcal{P}^j(y))*\psi_{\beta}
	\begin{cases}
...
The last requirement uses the fact that the average operation can also be defined as a convolution product between the filter function and the flow field quantities (\citet{marle1982macroscopic}):
$$
\means{\psi_{\beta}}|_{\mathbf{x}} = \dfrac{1}{V} \int_{\volb(\mathbf{x})} m(\mathbf{y}) \psi_{\beta}(\mathbf{x}+\mathbf{y}, t) d \, \volb = m*\psi_{\beta}
$$

The importance of the filter function shape is clear; however older literature works that had implicitly used $m^{\sqcap}$ are not wrong.
As a matter of fact, if the assumption of well behaved fields holds (it means that the equation \eqref{eq:well_behaved} can be verified a posteriori) than the homogenized equations are the correct one.
...
In the following derivation of the equation no filter function is used inside the averaged operators, in order to not make the notation heavier.
In any case whenever special hypothesis on the filter are needed, it is indicated in the following text.

\subsection{Theorems involving derivatives of spatial averaged quantities}

The purpose of these theorems is to be able to invert the derivative and volume average operation.
...
\begin{corollary}
	Applying the theorem \eqref{th:spat_avg} to a constant field $\psi_{\beta} = 1$ the following relation can be retrieved:
...
Before continue the development we add all the various terms together:
...
For the momentum equation we start from the temporal derivative term that become:
$$
...
First of all is important to pinpoint that the intrinsic momentum equation represent the force per unit volume. It means that it explicitly depends on the porosity of the medium; that of course can change (this is why it has terms involving gradients of the porosity).
...
In the next paragraphs a closure formulations of these terms is presented. In order to do be clear these two terms are named as \textit{sub-filter stresses} $\boldsymbol{\zeta}$ and \textit{microscopic force} $\mathbf{F}^m$:
...
The term $\mathbf{F}^m$ is referred as \textit{surface filter} since it is defined as an integral over the fluid-solid interface for the micro-scale fields; in this manner the perturbation fields are filtered out. It physically represent the force per unit mass that the fluid exert on the solid porous media.

There is no simple representation for $\mathbf{F}^m$ if we include the terms that presents gradients of the porosity ($\nabla \varepsilon$).
Although since we are interested in developing a \textbf{local} closure problem, that will depend on the geometry of each REV, is possible to neglect these terms.
This means that the closure problems are not correct at the interface between a porous medium and a free fluid. However in the last chapter we show that good results can still be achieved using the same closure problems.

In order to develop a closure problem we start by recalling the decomposition by \citet{gray1975derivation}:
\begin{equation}
...
We subtract this last equation from the continuity equation valid for the point velocity:
...
expanding the first velocity with the decomposition \eqref{eq:gray}, and making the proper simplification, we obtain the continuity equation for the perturbations:
...
To continue the development we first divide the momentum equation present in system \ref{eq:vans_mom_1} by the permeability $\varepsilon$, and we also apply the hypothesis of constant porosity:
\begin{eqnarray}
...
Using the same procedure as above, we subtract the above momentum equation from the point field one obtaining:
\begin{eqnarray}
...
Now in order to simplify the above equations we introduce the following length-scale estimates:
...
The last relation is based on the fact that the porosity varies on scale $\delta$ that is much greater than $\ell$, this means $\ell \ll \delta$. \citet{valdes2013velocity} and \citet{ochoa1995momentum} make this argument considering that $\delta$, in the case of an interface between a porous medium and a free fluid, has the size of the zone in which the porosity varies.
...
\citet{whitaker1996forchheimer} state clearly that there is no easy way to define a \textit{local} closure problem when the relation $\ell \ll \delta$ does not holds. In any case here we suppose that the relation hold in order to fulfill the derivation of the local closure problem; although we keep in mind that the problem is formally correct only far from regions where the porosity can change.

Analyzing the estimates order of magnitude is possible to neglect some of the terms in the previous momentum equation \eqref{eq:vans_mom_3}:
\begin{eqnarray}
...
In the last estimation we have assumed that the time scale associated respectively with the micro and macro-scale are $t = \ell/\meani{\vb}$ and $T =L/\meani{\vb}$.
This assumption imply that the perturbation problem is \textit{quasi-stationary}, the assumption is physically held by the fact that the perturbation field can be considered steady from the macroscopic evolution perspective (\citet{davit2013homogenization} and \citet{zhu2014study}).
I can also be notice that in the above simplifications we have neglected terms that contains the small parameter $\epsilon$ (or powers of this term); this results is coherent with the multiple scale theory in which only zero order terms are used in the local closure problem formulation.
...
The boundary condition at the interface is valid only for rigid porous media. In this section we derive the closure problem considering the solid phase as rigid, although in section \ref{ph:moving} we show how to extend the model in case of moving porous media.
...
Such hypothesis is consistent with periodic ordered porous media we have already neglected macroscopic field variation inside the REV (see paragraph \ref{ch:appendix_a}).
...
In the above problem the averaged condition $\meani{\vbt} = 0$ is used to make the solution unique, that otherwise would be defined up to a constant.

Now we introduce the closure variables $\mathbf{R}$ (tensor) and $\mathbf{r}$ (vector), for which we made the hypothesis that:
\begin{eqnarray}
...
Is very important to pinpoint that the last assumption is crucial since a linear correlation between the micro and macro-scale fields is implied.
However this relation can be space dependent ($R$ and $r$ are function of the space coordinate $x$) as explored in chapter \ref{ch:4}.
...
It is possible to prove that with the above problem definition $\boldsymbol{\xi}$ is zero and $\gamma$ a constant (\cite{whitaker1996forchheimer}).
The above closed formulation, for the two quantities $\mathbf{R}$ and $\mathbf{r}$, is difficult to solve computationally since it is an integral-differential equation.
...
We chose to decompose $\mathbf{R}$ and $\mathbf{r}$ as:
...
We are free to define $\mathbf{B}$ by means of the following system:
...
We want to highlight that the problems for $R$, $B$ and $C$ are a free definition and they not arise from the direct substitution of \ref{eq:closure_viab_divison} and 	\ref{eq:closure_viab_hp} into the system \ref{eq:vans_mom_2}. Injecting the decomposition \ref{eq:closure_viab_hp} inside the surface filter $\mathbf{F}^m$ we get:
...
It is possible to use simplified regression that permits to by-pass the \textit{local} closure problem computation and get directly the tensors $\mathbf{K}$ and $\mathbf{F}$.
...
However the above relationship are always based on some regression from experiments or computation, and they are parameterized usually with the porosity and some geometrical characteristic of the medium.
...
\end{equation}
in which is clear the equivalence between the description by means of the perturbation fields and the one that uses only the macroscopic fields.
...
$$\boldsymbol{\zeta} = \nabla \cdot \left(\means{\vbt \vbt}\right)$$
Although we have already seen in the previous paragraph that this term can be neglected, based on some length-scale argument, here we want to explain briefly what this term represent and possibly when it can become important.
...
\begin{itemize}
	...
	\item \textit{turbulent dispersion:} is caused by the subfilter scales eddies that creates at the pore scale. This type of additional diffusivity can be anisotropic. For example in case of fibrous porous media the vertical penetration (and breakdown of eddies) is much higher than the horizontal one.
\end{itemize}
...
Although, we speculate that this term can become important in situations involving elastic porous where sweep and injection of fluid can be observed at the interface.
This statement is supported by \citet{finnigan2000turbulence} and \citet{de2008effects} that have shown the turbulence spectrum modification in case of canopy flow. Possibly the sub-filter stresses term can model this high frequency vorticies that shift the spectrum.
...
These works have all in common the fact that a certain slip in specified at the interface, for example the \citet{beavers1967boundary} condition reads:
$$
...
Others proposition change and extend this formulation but basically still impose a velocity jump at the interface, as a function of a parameter (here $\alpha$) needed to fit the experimental data.
...
As a matter of fact either methods require a parameter to close the formulation. The advantage, using penalization method, is that now the parameter needed is the spatial distribution of the porosity field that is trivial to compute when the geometry of the medium is known.
Although is still not clear how to vary the permeability in the transition zone. Most of the authors propose a sharp jump from the porous media value and the free fluid one. Neglecting the variation of permeability at the transition zone appear to be acceptable, even though example of linear variation of this terms exists \citet{caltagirone1994interaction}.
\citet{hussong2011continuum} made a direct comparison (with a DNS simulation) for the porosity treatment concluding that the variation of the permeability are very important to have a good comparison with high fidelity computation.
...
In the following work we adopt the penalization approach with the porosity variation computed directly from the geometry of our fibrous medium and a steep variation of the effective permeability at the interface. However in chapter 5 we show some details and results on this approach.
...
This simple modification can be included in an \textit{hybrid homogenization approach}. As presented in paragraph \ref{ch:model_porous}, in order to describe the dynamic of a poroelastic layer we need a model for the fluid part, and one for the solid plus an interface condition. Nevertheless since the dynamic of the solid is usually computationally cheap there is no need to use an homogenized model for this part. Here we propose an approach that is transparent to whatever solid model one want to use. The pseudo-code fo the macroscopic algorithm is the one in \ref{algo:vans_elastic}.
\vspace{0.5cm}
...
In the above sections we have briefly talked about the results in equation \ref{eq:well_behaved} that we recall here:
...
As we recall from section \ref{ch:filter} the triangle shaped filter almost satisfy this hypothesis. The function $m^{\triangle}$ guarantee a second order closure it means that $\dfrac{1}{2} \means{\mathbf{y}\mathbf{y}}|_{\mathbf{x}} :  \nabla \nabla \means{\psi_{\beta}}|_{\mathbf{x}}$ is a constant. Further manipulation can show that it is also negligible ($O(\epsilon^2)$).

\section{Conclusions}
We have shown in this chapter how to formally derive the homogenized version of the Navier-Stokes equations. Also some extensions in case of elastic porous medium has been introduced. We have focused also in the closure problem for the microscopic force since this topic is further developed in chapter 4.
Although the average volume method is not new in the homogenization field we think that this chapter group also the newest works in literature and can form a basis for better understand the next chapters.
