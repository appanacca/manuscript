...
This results can be simplified; in fact the last integral is zero due to the time derivation of a constant field and the first term can be further developed, obtainig finally the averaged continuity equation \eqref{eq:avg_1}.
...
This is still not the superficial averaged version of the momentum equation, since it has the presence of the non-homogeneous term $\means{\vb\vb}$.


\subsection{Length scale decomposition}
In order to finally get the superficial average version of the problem \eqref{eq:NS} we make use of the decomposition proposed by \citet{gray1975derivation}:
...
\begin{equation}
\means{\vb\vb} = \means{\means{\vb}\means{\vb}} +2\means{\means{\vb}\vbt} +\means{\vbt \vbt} = \varepsilon \meani{\vb}\meani{\vb} +\means{\vbt\vbt}
\label{eq:dec_1}
\end{equation}

Now we can display the superficial average Navier-Stokes system:

\begin{eqnarray}
\begin{cases}
\derp{\means{\vb}}{t} + \nabla \cdot (\varepsilon^{-1} \means{\vb}\means{\vb}) + \nabla \cdot (\means{\vbt\vbt}) = -\dfrac{1}{\rho_{\beta}} \nabla \means{\pb} + \nub\nabla^2 \means{\vb} + \nonumber \\
\qquad \qquad \qquad \qquad +\dfrac{1}{V} \displaystyle\int_{A_{\beta \sigma}} \left( -\dfrac{\pb}{\rho_{\beta}} \mathbf{I} + \nub \nabla \vb  \right)\cdot \nbs \;dA \nonumber \\
 \nabla \cdot \means{\vb} + \derp{\varepsilon}{t} =0
\end{cases}
\label{eq:NS_2}
\end{eqnarray}

It is not closed since it has the presence of the point fields in the integral term and the perturbation fields in one of the convective terms.
In order to overcome this problems in the next section the intrinsic version of these equation is computed. 

\subsection{Intrinsic average form}
We can also write the above problem using the intrinsic average.
...
&&+ \dfrac{1}{V} \int_{A_{\beta \sigma}} \left(-\dfrac{\pbt}{\rho_{\beta}} \mathbf{I}  + \nub \nabla \vbt \right)\cdot \nbs \;dA
\label{eq:vans_mom_1}
\end{eqnarray}
