...
The size and shape is very important in the developing of the averaged equation, however its definition (\textit{reference elementary volume} REV) is presented in the following paragraph when needed.
...
Inside the average operators we can further use a weight function ($m$)
If $m$ has the form:
\begin{eqnarray}
m(\mathbf{y}) 
\begin{cases}
...
It can be shown that the appropriate formulation of this function is needed in case of ordered porous media to avoid geometrical fluctuations of the averaged quantities \citet{quintard1994transport4}, \cite{quintard1994transport2}.
