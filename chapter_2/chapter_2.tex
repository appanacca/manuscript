...
\begin{equation}
 \means{\nabla \cdot (\rho_{\beta}\vb)} = \nabla \cdot \means{ (\rho_{\beta}\vb)} + \dfrac{1}{V} \int_{A_{\beta \sigma}} \rho_{\beta}  \vb \cdot \nbs \;dA
 \label{eq:avg_1}
\end{equation}
...
\begin{eqnarray}
&&\means{\dfrac{\partial \rho_{\beta}}{\partial t}} = \dfrac{1}{V} \int_{V} \dfrac{\partial \rho_{\beta}}{\partial t} \; dV = \dfrac{1}{V} \left[  \dfrac{\partial}{\partial t} \int_{V} \rho_{\beta} \; dV -  \int_{A_{\beta \sigma}} \rho_{\beta} (\ws \cdot \nbs ) \;dA   \right]  \nonumber \\
&& = \dfrac{\partial \means{\rho_{\beta}}}{\partial t} -\dfrac{1}{V}  \int_{A_{\beta \sigma}} \rho_{\beta} ( \ws\cdot  \nbs) \;dA 
\label{eq:avg_2}
\end{eqnarray}

The boundary condition at the interface $\ws = \vb$ imply that the integrals in \ref{eq:avg_1} is equal to the integral in \ref{eq:avg_2} so we end up with:
...
The boundary condition at the interface $\ws = \vb$ imply that the integrals in the last two equations are equals so we end up with the total left end side of the momentum equation:
...
\begin{equation}
\means{\vb\vb} = \means{\means{\vb}\means{\vb}} +2\means{\means{\vb}\vbt} +\means{\vbt \vbt}
\label{eq:dec_1}
\end{equation}
...
\begin{equation}
\means{\vb\vb} = \varepsilon \meani{\vb}\meani{\vb} +\means{\vbt \vbt}
\end{equation}
