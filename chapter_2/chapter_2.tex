...
Now the perturbed field has to be express as a function of some averaged values. Let us introduce the closure tensor $\mathbf{R}$ and  the closure vector $\mathbf{r}$ as:
\begin{eqnarray}
...
\citet{whitaker1996forchheimer} propose to define $\mathbf{R}$ and $\mathbf{r}$ via the following problem:
...
In order to simplify the problem, it is further decomposed into two parts, the solution of the first one gives us the \textit{permeability tensor} and the solution of the second one the \textit{Forchheimer tensor}. The variables $\mathbf{R}$ and $\mathbf{r}$ are further decomposed as:
