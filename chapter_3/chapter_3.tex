...
%in the stability equations has but a negligible influence on the leading instability mode. 
In section \ref{sec:stability} of the introduction we have already introduced the stability problems for flow through porous media.
...
%Nonetheless, in Appendix A we provide some considerations on how $C_D$ affects the mean flow in the model used here.
...
An example of direct and adjoint eigenfunctions is provided in figure \ref{fig:3}, both in the viscous case
...
The adjoint equations in this case are the same as system \eqref{eq:uvp}, without the terms containing $1/Re$
...
Stability problems such as the first one considered here can be approached with the
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%
%The discretization operation transform the operator $ {L}$ into a matrix $\mathbf{A}$ and of course do the same things to the unknown functions that becomes vectors.
%
%\begin{table}[H]
%	\begin{center}
%		\begin{tabular}{|c|c|}
...
%
%\begin{equation}
%\langle q;  {L} q \rangle = \langle { {L}}^{\dagger} q^{\dagger} ; q \rangle
%\end{equation}
%
...
%$ {M}$ is the matrix representation of the weights and is symmetric and positive defined.
%
%In order to compute and solve the adjoint equation one could proceed as follow:
%
%\begin{itemize}
%	\item The direct problem is defined in the continuous space as $ {L} q = 0$
...
%\begin{split}
%\langle q;  {L} q \rangle =& \langle { {L}}^{\dagger} q^{\dagger} ; q \rangle \\
%\Rightarrow \int_{0}^{y_{\infty}} \overline{q^{\dagger}} \;  {L} q  \; dy =& \int_{0}^{y_{\infty}} \overline{ { {L}}^{\dagger} q^{\dagger}}  \; q  \; dy 
%\end{split}
%\end{equation}
%
...
%\begin{equation}
%\begin{split}
%\langle q^{\dagger};  {L} q \rangle =& \langle { {L}}^{\dagger} q^{\dagger} ; q \rangle \\
