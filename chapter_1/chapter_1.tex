...
The micro-scale behavior associated with the length scale 

\begin{equation}
\means{\psi} = \dfrac{1}{V} \int_{V} \psi d \, V.
\label{eq:avg_superficial_intro}
\end{equation}


\begin{equation}
\psi =   \underbrace{ \means{\psi} }_\text{O($L$)}  +  \underbrace{\tilde{\psi} }_\text{O($\ell$)},
\label{eq:vans_decomp}
\end{equation}



Since this is the method chosen to solve our problem it will be detailed explained in the chapter \ref{ch:vans}.
